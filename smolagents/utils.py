...
def parse_json_blob(json_blob: str) -> tuple[dict[str, str], str]:
    ...
    #     )
    """
    Finds the first occurrence of a valid JSON object or array within a string,
    parses it, and returns the parsed data and the text that came before it.

    Args:
        text: The string to search for a JSON blob.

    Returns:
        A tuple containing the parsed JSON data (dict or list) and the preceding text.

    Raises:
        ValueError: If no valid JSON object or array is found in the string.
    """
    # Find the starting position of the first '{' or '['
    # This makes the search more efficient by not trying to parse from the very beginning
    # if the string doesn't start with a JSON structure.
    first_brace = json_blob.find('{')
    first_bracket = json_blob.find('[')

    # Determine the first potential start index of a JSON blob
    if first_brace == -1 and first_bracket == -1:
        raise ValueError("The model output does not contain any JSON blob start token ('{' or '[').")
    ...
    # The text before the potential JSON start
    preceding_text = json_blob[:start_index]

    # The substring that starts with the potential JSON
    json_candidate_str = json_blob[start_index:]

    try:
        # Use the raw_decode method to parse the first valid JSON object/array
        # from the candidate string. It correctly handles nested structures.
        decoder = json.JSONDecoder()
        json_data, _ = decoder.raw_decode(json_candidate_str)
        return json_data, preceding_text

    except json.JSONDecodeError as e:
        # Provide a more robust error message
        error_pos = start_index + e.pos
        raise ValueError(f"The JSON blob you used is invalid due to the following error: {e}.\n"
                         f"Original json_blob was: {json_blob}\n"
                         f"Decoding failed near this part of the text:\n"
                         f"'{json_blob[max(0, error_pos - 10) : error_pos + 10]}'.")
...
def instance_to_source(instance, base_cls=None):
    ...
    class_attrs = {
        name: value
        for name, value in cls.__dict__.items() if not name.startswith("__") and not name == "_abc_impl" and not callable(value)
    ...
    methods = {
        ...
        if callable(func) and (not base_cls or not hasattr(base_cls, name) or (isinstance(func, (
            staticmethod, classmethod)) or (getattr(base_cls, name).__code__.co_code != func.__code__.co_code)))
    }
