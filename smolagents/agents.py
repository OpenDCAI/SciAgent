...
class MultiStepAgent(ABC):
    ...
    def _setup_managed_agents(self, managed_agents: list | None = None) -> None:
        ...
        self.managed_agents = {}
        if managed_agents:
            assert all(agent.name and agent.description
                       for agent in managed_agents), ("All managed agents need both a name and a description!")
    ...
    def _setup_tools(self, tools, add_base_tools):
        assert all(isinstance(tool, BaseTool) for tool in tools), ("All elements must be instance of BaseTool (or a subclass)")
...
{str(additional_args)}."""
    ...
    def _run_stream(
        ...
        while not returned_final_answer and self.step_number <= max_steps:
            ...
            if self.planning_interval is not None and (self.step_number == 1 or
                                                       (self.step_number - 1) % self.planning_interval == 0):
        ...
        if is_first_step:
            ...
            plan_update_pre = ChatMessage(
                ...
                content=[{
                    ...
                    "text",
                    "text":
                    populate_template(self.prompt_templates["planning"]["update_plan_pre_messages"], variables={"task": task}),
                }],
            )
    ...
    def _step_stream(self, memory_step: ActionStep) -> Generator[ChatMessageStreamDelta | ToolCall | ToolOutput | ActionOutput]:
        """
        ...
        messages.append(
            ChatMessage(
                ...
                content=[{
                    ...
                    "text",
                    "text":
                    populate_template(self.prompt_templates["final_answer"]["post_messages"], variables={"task": task}),
                }],
            ))
        try:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if hasattr(self, "authorized_imports"):
            requirements.update(
                {package.split(".")[0]
                 for package in self.authorized_imports if package not in BASE_BUILTIN_MODULES})
...
class ToolCallingAgent(MultiStepAgent):
    ...
    def process_tool_calls(self, chat_message: ChatMessage, memory_step: ActionStep) -> Generator[ToolCall | ToolOutput]:
        ...
        for chat_tool_call in chat_message.tool_calls:
            tool_call = ToolCall(name=chat_tool_call.function.name,
    ...
    def _substitute_state_variables(self, arguments: dict[str, str] | str) -> dict[str, Any] | str:
        ...
        if isinstance(arguments, dict):
            return {key: self.state.get(value, value) if isinstance(value, str) else value for key, value in arguments.items()}
        return arguments
    ...
    def execute_tool_call(self, tool_name: str, arguments: dict[str, str] | str) -> Any:
        ...
        if tool_name not in available_tools:
            raise AgentToolExecutionError(f"Unknown tool {tool_name}, should be one of: {', '.join(available_tools)}.",
                                          self.logger)
        ...
        try:
            ...
            if is_managed_agent:
                error_msg = (f"Error executing request to team member '{tool_name}' with arguments {str(arguments)}: {e}\n"
...
class CodeAgent(MultiStepAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.code_block_tags = (code_block_tags if isinstance(code_block_tags, tuple) else
                                ("```python", "```") if code_block_tags == "markdown" else ("<code>", "</code>"))
    ...
    def create_python_executor(self) -> PythonExecutor:
        if self.executor_type == "local":
            ...
            return remote_executors[self.executor_type](self.additional_authorized_imports, self.logger, **self.executor_kwargs)
    ...
    def initialize_system_prompt(self) -> str:
        system_prompt = populate_template(
            ...
            variables={
                ...
                "managed_agents":
                self.managed_agents,
                "authorized_imports": ("You can import from any package you want."
                                       if "*" in self.authorized_imports else str(self.authorized_imports)),
                "custom_instructions":
                self.instructions,
    ...
    def _step_stream_vanilla(
            self, memory_step: ActionStep) -> Generator[ChatMessageStreamDelta | ToolCall | ToolOutput | ActionOutput]:
        """
        Perform one step in the ReAct framework: the agent thinks, acts, and observes the result.
        Yields ChatMessageStreamDelta during the run if streaming is enabled.
        At the end, yields either None if the step is not final, or the final answer.
    ...
    def _step_stream(self, memory_step: ActionStep) -> Generator[ChatMessageStreamDelta | ToolCall | ToolOutput | ActionOutput]:
        ...
        """
        # --- Stage 1: Generate Model Output (The "Thought" and "Code") ---
        memory_messages = self.write_memory_to_messages()
        input_messages = memory_messages.copy()
        memory_step.model_input_messages = input_messages

        # FIX 1: Define MULTIPLE stop sequences. The agent will stop at the FIRST one it encounters.
        # This makes it robust to the LLM ignoring the preferred format.
        stop_sequences = [self.code_block_tags[1], "```"]

        try:
            # The generation logic itself remains the same, but now has more robust stopping conditions.
            chat_message: ChatMessage = self.model.generate(
                input_messages,
                stop_sequences=stop_sequences,
                # ... any other args like response_format ...
            )
            output_text = chat_message.content if chat_message.content else ""

            # This logging step is fine.
            self.logger.log_markdown(
                content=output_text,
                title="Output message of the LLM:",
                level=LogLevel.DEBUG,
            ...
            # The insurance policy of adding back the closing tag is now more complex.
            # We don't know which one it stopped on. The robust parser `parse_code_blobs` handles this better.
            # So we remove the manual addition of the tag.
        ...
        try:
            # FIX 2: Revert to using the more robust `parse_code_blobs` function.
            # It is specifically designed with a fallback to handle the Markdown format,
            # which is exactly what we need for this safety net.
            if self._use_structured_outputs_internally:
                # This logic branch is for a different agent type, but for consistency, we'll assume
                # it also needs robust parsing.
                code_action_json = json.loads(output_text)
                # We still need to parse the code within the JSON
        ...
        # No changes needed here. This part of the code receives the successfully parsed code_action.
